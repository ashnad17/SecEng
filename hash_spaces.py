...
# Compute the hash of the first input
hash1 = sha256_hash(input1)

# Modify the second input to match at least 3 of the last digits of the first hash
# Modify the second input by adding spaces to match at least 3 of the last digits of the first hash
spaces = ""
while True:
    modified_input2 = input2 + spaces
    hash2 = sha256_hash(modified_input2)
    
    # Check if the last 3 digits of the hash match
    if hash1[-2:] == hash2[-2:]:
        break
...
print(f"Modified Input 2: '{modified_input2}'")
